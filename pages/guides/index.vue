...
<template>
  <div class="our-guides bg-grey-700 ">
    ...
    <div
      ...
      <p class="max-w-264 sm:max-w-full text-mobile-h1 sm:text-desktop-h1 leading-4sm sm:leading-3sm text-terciary mb-24 sm:mb-32 font-bold">Meet Our Guides</p>
      <!-- Tags -->
      <tags
        :tags.sync="citiesList"
        @changeTab="changeTab" />
    </div>

    <!-- Guide Start -->
    <div v-if="filteredGuides" class="w-95/100 lg:w-5/6 lg:max-w-1180 mx-auto  mt-30">
      <div v-for="(row, indexRow) in rows" :key="indexRow" class="hidden sm:flex items-center flex-wrap justify-between">
        <!-- Guide Cards -->
        <div
        ...
        >
          <guide-card
            :background-url="`url(${ mediaUrl + 't_thumbnail/' + guide.photo.desktopUrl.trim() || require(`@/static/img/no-image.png`) })`"
        ...
        <transition name="slide">
          <guide-more-info
            ...
            :image-u-r-l="`${mediaUrl}t_thumbnail/${filteredGuides[activeIndex].photo.desktopUrl.trim()}` || require(`@/static/img/no-image.png`)"
          />
        </transition>
      </div>
      <!-- Guide Carousel -->
    ...
    <div
      ...
      <div class="max-w-1180 w-full mx-auto px-16 md:px-30 py-32 md:py-64">
        <div class="flex items-center md:items-start flex-col-reverse md:flex-row">
          <div class="max-w-full md:max-w-600 w-full mr-0 md:mr-88">
            <div class="h-152 md:h-312">
              <img
                :src="`${mediaUrl}t_header/${getSiteInfo.aboutUsBarImage.desktopUrl.trim()}` || require(`@/static/img/no-image.png`)"
            ...
            <div
              ...
              <span class="text-mobile-body sm:text-desktop-h4 leading-1sm sm:leading-normal text-grey-40">
                ...
                {{ getSiteInfo.aboutUsBarImageSubtitle }}&nbsp;<span class="text-blue-200" />
              </span>
            </div>
          </div>
          <div class="max-w-full md:max-w-440 w-full mb-40 md:mb-0">
            ...
            <p class="text-terciary text-mobile-h3 md:text-desktop-h3 leading-3sm md:leading-5sm mb-24">{{ getSiteInfo.aboutUsBarText }}</p>
            <nuxt-link :to="`/${getHalifaxSlug}`">
              <app-button type="default" class="text-mobile-h3 md:text-desktop-h3 leading-3sm md:leading-5sm px-46 py-1050 md:py-12 rounded-8">
    ...
    <div
      ...
      <p class="text-mobile-h1 leading-4sm md:text-desktop-h1 font-bold text-secondary pt-32 md:leading-3sm mb-16 px-16 sm:px-0">Live the experience of our tours</p>
      <p class="text-desktop-h3 leading-5sm text-terciary mb-32 font-bold px-16 sm:px-0">The beginning of your adventure!</p>
      <p class="text-terciary mb-24 text-desktop-h4 leading-sm px-16 sm:px-0">Day Tours</p>
      <div class="our-tour-experience__wrap overflow-x-auto overflow-y-hidden">
        <div class="our-tour-experience__scroll inline-block mx-16 lg:mx-0">
          <div class="tour-card-inner justify-between lg:justify-start flex flex-wrap">
            <!-- Tour card -->
            <tour-card
      ...
      <div class="flex justify-end mt-20 md:mt-40 mb-32 px-16 md:px-0">
        <nuxt-link to="/tours">
          <app-button type="arrow" class="flex items-center text-secondary text-mobile-h3 sm:text-desktop-h3">
            <span>See All Day Tours {{ `(${getDayTours.length})` }} </span>
            <span class="icon-arrow-bold ml-8 block rotate-180 text-mobile-note" />
          </app-button>
        </nuxt-link>
      </div>
      <p class="text-terciary mb-24 text-desktop-h4 leading-sm px-16 sm:px-0">Multi Day Tours</p>
      <div class="our-tour-experience__wrap overflow-x-auto overflow-y-hidden">
        <div class="our-tour-experience__scroll inline-block mx-16 lg:mx-0">
...
export default {
  ...
  computed: {
    ...
    filteredGuides () {
      if (this.tabIndex) {
        return this.guides.filter(g => g.location.id === this.citiesList[this.tabIndex].id)
      } else {
        return this.guides
      }
    },
    ...
    rows () {
      let guides
      if (this.tabIndex) {
        guides = this.guides.filter(g => g.location.id === this.citiesList[this.tabIndex].id)
      } else {
        guides = this.guides
      }
      const allGuides = []
      ...
      for (let i = 0; i < guides.length; i++) {
        ...
        totalPushed++
        if (currentNumberOfGuides === this.guidesPerRow | totalPushed === guides.length) {
  ...
  methods: {
    ...
    changeTab ({ index }) {
      for (const item of this.citiesList) { item.active = false }
