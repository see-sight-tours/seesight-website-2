<template>
  <div class="bg-grey-700">
    <div
      class="max-w-1180 w-full mx-auto pl-16 pr-0 sm:px-30 py-40 sm:py-48 flex items-center justify-start flex-col md:flex-row"
    >
      <div
        v-if="citiesPage"
        class="max-w-full md:max-w-280 w-full flex flex-col mr-0 md:mr-40 mb-36 md:mb-0 pr-16 sm:pr-0"
      >
        <h2
          class="text-terciary text-mobile-h1 sm:text-desktop-h1 font-bold leading-4sm sm:leading-3sm mb-16 sm:mb-24"
        >
          {{ citiesPage && citiesPage.discoverCitiesBarHeader }}
        </h2>
        <p
          class="text-grey-400 text-mobile-h3 sm:text-desktop-h3 leading-3sm leading-5sm mb-32 sm:mb-16"
        >
          {{ citiesPage && citiesPage.discoverCitiesBarDescription }}
        </p>
        ...
        >
          <nuxt-link :to="`/${city.slug}`">
            <div class="h-140 lg:h-280 mb-16">
              <img
                :src="
                  `${mediaUrl}t_thumbnail/${city.featureImages[0] && city.featureImages[0].desktopUrl.trim()}` ||
            ...
            <h3
              class="text-terciary text-desktop-h3 leading-5sm font-bold mb-8"
            >
              {{ city.name }}
            </h3>
            <p class="text-terciary text-desktop-h6 leading-6sm">
              {{ city.snippet }}
            </p>
          </nuxt-link>
        </div>
