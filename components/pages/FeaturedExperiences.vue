<template>
  ...
  {{productType}}
    ...
    >
      <h2
        ref="pageHeader"
        class="w-full sm:w-auto text-mobile-h3 md:text-desktop-h2 text-terciary font-bold leading-2sm px-16 md:px-0 mb-0"
      >
      ...
      >
        <div v-if="cities" class="flex items-center justify-end">
          <p
            class="text-grey-400 text-mobile-h5 sm:text-desktop-h5 leading-6sm sm:leading-1sm mr-16"
          >
    ...
    <div
      v-if="products"
      class="featured-experiences__wrap overflow-x-auto overflow-y-hidden"
    >
      <div class="featured-experiences__scroll inline-block mx-16 lg:mx-0">
        ...
        >
          <tour-card
            v-for="tourCardItem in products.slice(0, amount)"
            :key="tourCardItem.id"
    ...
    <div
      v-if="!filterType && tags[activeIndex].link"
      class="flex justify-end mt-20 md:mt-48 px-16 md:px-0"
    >
      <a :to="tags[activeIndex].link">
        <app-button type="arrow" class="flex items-center text-secondary">
          <span v-if="isShowMore" @click="showMore"
            >See all {{ tags[activeIndex].name }} ({{ productsAmount }})</span
          >
          <span v-else @click="showLess">Show Less</span>
          <span
            class="icon-arrow-bold block text-mobile-note leading-default rotote-0 ml-8"
          />
        </app-button>
      </a>
    </div>
...
import { productsByProductType } from '@/api/queries/product'
...
export default {
    ...
    return {
      ...
      tags: [
        ...
        },
        {
          ...
          productType: "MULTIDAY",
          link: "/multi-day-tours",
          active: false,
        },
        {
          name: "Airport Transfer",
          productType: "AIRPORTPICKUP",
  ...
  methods: {
    ...
    filterProducts () {
      ...
      this.$apollo.query({ query: productsByProductType, variables: { productType: this.productType } })
